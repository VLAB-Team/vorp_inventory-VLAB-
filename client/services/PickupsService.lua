...
local ItemPropMap = LoadResourceFile(GetCurrentResourceName(), 'config/dropitems.lua') and assert(load(LoadResourceFile(GetCurrentResourceName(), 'config/dropitems.lua')))()
...
PickupsService.createPickup           = function(name, amount, metadata, weaponId, id)
	...
	-- Determina il modello di prop basato sull'item, con fallback su "cottonbox"
	local pickupModel = ItemPropMap[name] or "P_COTTONBOX01X" -- Usa "P_COTTONBOX01X" come modello di default
...
PickupsService.createMoneyPickup      = function(amount)
	...
	-- Usa il modello di "moneybag" o "P_COTTONBOX01X" come fallback
	local pickupModel = ItemPropMap["moneybag"] or "P_COTTONBOX01X"
...
PickupsService.createGoldPickup       = function(amount)
	...
	-- Usa il modello di "goldbar" o "P_COTTONBOX01X" come fallback
	local pickupModel = ItemPropMap["goldbar"] or "P_COTTONBOX01X"
	...
	if value == 1 then
		if WorldPickups[data.obj] == nil then
			...
			local pickup = Pickup:New({
				...
				prompt   = Prompt:New(0xCEFD9220, T.TakeFromFloor, PromptType.StandardHold, promptGroup),
				uid      = data.uid
